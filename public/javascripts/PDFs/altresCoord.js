	...
	models.Alumne.findById(alumneId, function(error, alumne){
		if (error) {
			return res.json(error);
				...
				var tempFile = ('temp/altresCoord.pdf');
				...
				var nom = alumne.nomAlumne + ' ' + alumne.cognomAlumne1 + ' ' + alumne.cognomAlumne2;
					...
					//doc.text('ACTUACIÓ');
					//doc.moveDown(0.5);
					doc.text("Data de la coordinació: " + alumne.segAltresCoord[i].date);
					doc.moveDown(1.5);

					doc.text('Informació rebuda');
					doc.moveDown(0.5);
					var body = alumne.segAltresCoord[i].body;
